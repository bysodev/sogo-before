...
export const userStore = create<State>()((set) => ({
    ...
    fetchLoginUser: async (username: string, password: string ) => {
        ...
        try {
            ...
            if (response.status === 200) {
                const {access_token, respuesta, username, email} = await response.json();
    ...
    fetchRegisterUser: async (username: string, password: string, email: string ) => {
        var myHeader = new Headers();
        myHeader.append("Accept", "application/json");
        ...
        try {
            ...
            if (response.status === 201) {
                ...
                try{
                    ...
                    if ( response.ok ){
                        showSuccessMessage('El correo de verificación fue revisado exitosamente')
                    }
                } catch(e){
                    showErrorMessage('El correo de verificación no pudo ser enviado!! Comuniquese con la administración')
                }
            ...
            console.error(error);
            showErrorMessage('Fallo al registrarse, comuniquese con la administración')
        }
        // console.log(process.env.API_BACKEND)
        // const res = await fetch('http://localhost:3000/api/user/login');
    },
